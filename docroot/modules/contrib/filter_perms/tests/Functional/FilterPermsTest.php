...
class FilterPermsTest extends BrowserTestBase {
  ...
  /**
   * Tests saving permissions when the POST data contains no button info.
   *
   * Replicates the scenario reported in
   * https://www.drupal.org/project/filter_perms/issues/3609578, where the
   * Gin admin theme's sticky form actions JavaScript triggers the "Save
   * permissions" button in a way that omits its name/value ("op") from the
   * submitted POST data entirely. Core's FormBuilder then falls back to
   * treating the first registered button on the form as the triggering
   * element, which must be the Save button, not the Filter Permissions
   * button, or the submitted permission changes are silently discarded.
   */
  public function testSaveWithoutTriggeringButtonInPost() {
    $this->drupalLogin($this->admin);
    $this->drupalGet('admin/people/permissions');
    ...
    $permission_field = $this->authorRole . '[access user profiles]';
    $this->assertSession()->checkboxNotChecked($permission_field);

    // Collect every input on the permissions form exactly as rendered, but
    // omit all buttons (both "Save permissions" and "Filter Permissions"),
    // reproducing a POST submission with no triggering button information.
    $form = $this->assertSession()->elementExists('css', 'form.user-admin-permissions');
    $post_data = [];
    foreach ($form->findAll('css', 'input') as $input) {
      $type = $input->getAttribute('type');
      $name = $input->getAttribute('name');
      if ($name === NULL || in_array($type, ['submit', 'image', 'button'], TRUE)) {
        continue;
      }
      if (in_array($type, ['checkbox', 'radio'], TRUE)) {
        ...
        $post_data[$name] = $input->getAttribute('value') ?? '1';
      }
      else {
        $post_data[$name] = $input->getValue();
      }
    }
    // Check the permission we want to grant, as the browser would after a
    // user ticks the checkbox.
    $post_data[$permission_field] = '1';

    $response = $this->getHttpClient()->post($this->getUrl(), [
      'cookies' => $this->getSessionCookies(),
      'form_params' => $post_data,
    ]);
    $this->assertSame(200, $response->getStatusCode());

    // Reload and re-filter to inspect the persisted state.
    $this->drupalGet('admin/people/permissions');
    $this->submitForm($filter_edit, 'Filter Permissions');
    $this->assertSession()->checkboxChecked($permission_field);
