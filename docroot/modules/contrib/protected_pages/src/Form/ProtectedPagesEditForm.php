...
class ProtectedPagesEditForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $pid = NULL) {
    ...
    $path = $this->protectedPagesStorage->loadProtectedPage($field_path, $conditions, TRUE);
    $title = $this->protectedPagesStorage->loadProtectedPage($field_title, $conditions, TRUE);
    ...
    $form['rules_list']['path'] = [
      ...
      '#default_value' => $path,
      '#description' => $this->t('Enter relative drupal path. For example, "/node/5", "/new-events" etc. Use the \'*\'
      wildcard character to target multiple pages, e.g. "/new-events/*".  For all pages on your site, enter "/*".'),
      '#required' => TRUE,
    ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $page_data['title'] = Html::escape($form_state->getValue('title'));
