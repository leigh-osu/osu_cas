...
class Selective extends InOperator {
  ...
  protected function getOriginalOptions() {
    if (!isset($this->originalOptions)) {
      ...
      $original_plugin_definition = $original_filter->definition;

      // Add selective filter metadata while preserving proxied definition keys.
      $original_filter->definition = $this->definition + $original_plugin_definition;
      $original_filter->definition['id'] = $original_plugin_definition['id'] ?? $this->definition['proxy'];
      $original_filter->definition['class'] = $original_plugin_definition['class'] ?? get_class($original_filter);
      // Keep the proxied plugin provider when it is known.
      if (isset($original_plugin_definition['provider'])) {
        $original_filter->definition['provider'] = $original_plugin_definition['provider'];
      }
      $original_filter->value = $this->value;
      $original_filter->view = $this->view;

      if (is_callable([$original_filter, 'getValueOptions']) && !($original_filter instanceof Bundle)) {
