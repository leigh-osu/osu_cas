...
class ApStyleDateFormatterTest extends KernelTestBase {
  ...
  public function testMonthOnlySameMonthYear(): void {
    $year = (new DrupalDateTime('now'))->format('Y');
    $options = ['month_only' => TRUE];
    $start = new DrupalDateTime("$year-09-01");
    $end = new DrupalDateTime("$year-09-30");
   ...
   * Test month only in range when month and year is the same.
   *
   * Test when year is not the current year.
   */
  public function testNonCurrentYearMonthOnlySameMonthYear(): void {
    $year = '2024';
    $options = ['month_only' => TRUE];
    $start = new DrupalDateTime("$year-09-01");
    $end = new DrupalDateTime("$year-09-30");
    ...
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Sept. 2024', $result);
  }
  ...
  public function testMonthOnlyDifferentMonthsSameYear(): void {
    ...
    $options = ['month_only' => TRUE];
    $start = new DrupalDateTime("$year-09-01");
    $end = new DrupalDateTime("$year-10-01");
   ...
   * Tests when months are different and year is the same.
   */
  public function testNonCurrentYearMonthOnlyDifferentMonthsSameYear(): void {
    $year = '2024';
    $options = ['month_only' => TRUE];
    $start = new DrupalDateTime("$year-09-01");
    $end = new DrupalDateTime("$year-10-01");
    ...
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Sept. to Oct. 2024', $result);
  }
  ...
  /**
   * Tests format with display_noon_and_midnight option disabled.
   */
  public function testDisplayNoonAndMidnightDisabledRange() {
    $options = [
      'display_time' => TRUE,
      'display_noon_and_midnight' => FALSE,
    ];
    $start = (new DrupalDateTime('2023-12-15 09:00:00'))->getTimestamp();
    $end = (new DrupalDateTime('2023-12-15 12:00:00'))->getTimestamp();
    $result = $this->dateFormatter->formatRange(['start' => $start, 'end' => $end], $options);
    $this->assertEquals('Dec. 15, 2023, 9 a.m. to 12 p.m.', $result);

    $start = (new DrupalDateTime('2023-12-15 09:00:00'))->getTimestamp();
    $end = (new DrupalDateTime('2023-12-16 00:00:00'))->getTimestamp();
    $result = $this->dateFormatter->formatRange(['start' => $start, 'end' => $end], $options);
    $this->assertEquals('Dec. 15 to 16, 2023, 9 a.m. to 12 a.m.', $result);

    $start = (new DrupalDateTime('2023-12-15 12:00:00'))->getTimestamp();
    $end = (new DrupalDateTime('2023-12-15 15:00:00'))->getTimestamp();
    $result = $this->dateFormatter->formatRange(['start' => $start, 'end' => $end], $options);
    $this->assertEquals('Dec. 15, 2023, 12 p.m. to 3 p.m.', $result);

    $start = (new DrupalDateTime('2023-12-15 00:00:00'))->getTimestamp();
    $end = (new DrupalDateTime('2023-12-15 03:00:00'))->getTimestamp();
    $result = $this->dateFormatter->formatRange(['start' => $start, 'end' => $end], $options);
    $this->assertEquals('Dec. 15, 2023, 12 a.m. to 3 a.m.', $result);
  }
  ...
  public function testSeparatorHandling() {
    ...
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Jan. 10-20, 2012', $result);
  }

  /**
   * Tests separator handling with different month ranges.
   */
  public function testSeparatorHandlingDifferentMonths() {
    // Test range spanning different months.
    $start = new DrupalDateTime('2012-01-25 00:00:00');
    $end = new DrupalDateTime('2012-02-05 00:00:00');
    $timestamps = [
      'start' => $start->getTimestamp(),
      'end' => $end->getTimestamp(),
    ];
    ...
    $options = ['separator' => 'endash'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Jan. 25 &ndash; Feb. 5, 2012', $result);

    $options = ['separator' => 'to'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Jan. 25 to Feb. 5, 2012', $result);

    $options = ['separator' => 'hyphen'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Jan. 25-Feb. 5, 2012', $result);
  }

  /**
   * Tests separator handling with different year ranges.
   */
  public function testSeparatorHandlingDifferentYears() {
    // Test range spanning different years.
    $start = new DrupalDateTime('2012-12-28 00:00:00');
    $end = new DrupalDateTime('2013-01-05 00:00:00');
    $timestamps = [
      'start' => $start->getTimestamp(),
      'end' => $end->getTimestamp(),
    ];
    ...
    $options = ['separator' => 'endash'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Dec. 28, 2012 &ndash; Jan. 5, 2013', $result);

    $options = ['separator' => 'to'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Dec. 28, 2012 to Jan. 5, 2013', $result);

    $options = ['separator' => 'hyphen'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Dec. 28, 2012-Jan. 5, 2013', $result);
  }

  /**
   * Tests separator handling with time ranges.
   */
  public function testSeparatorHandlingWithTime() {
    $start = new DrupalDateTime('2012-01-10 09:30:00');
    $end = new DrupalDateTime('2012-01-10 17:45:00');
    $timestamps = [
      'start' => $start->getTimestamp(),
      'end' => $end->getTimestamp(),
    ];

    $options = ['display_time' => TRUE, 'separator' => 'endash'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Jan. 10, 2012, 9:30 a.m. &ndash; 5:45 p.m.', $result);

    $options = ['display_time' => TRUE, 'separator' => 'to'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Jan. 10, 2012, 9:30 a.m. to 5:45 p.m.', $result);

    $options = ['display_time' => TRUE, 'separator' => 'hyphen'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Jan. 10, 2012, 9:30 a.m.-5:45 p.m.', $result);
  }

  /**
   * Tests separator handling with time before date option.
   */
  public function testSeparatorHandlingTimeBeforeDate() {
    $start = new DrupalDateTime('2012-01-10 09:30:00');
    $end = new DrupalDateTime('2012-01-10 17:45:00');
    $timestamps = [
      'start' => $start->getTimestamp(),
    ...
    $options = ['display_time' => TRUE, 'time_before_date' => TRUE, 'separator' => 'endash'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('9:30 a.m. &ndash; 5:45 p.m., Jan. 10, 2012', $result);

    $options = ['display_time' => TRUE, 'time_before_date' => TRUE, 'separator' => 'to'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('9:30 a.m. to 5:45 p.m., Jan. 10, 2012', $result);

    $options = ['display_time' => TRUE, 'time_before_date' => TRUE, 'separator' => 'hyphen'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('9:30 a.m.-5:45 p.m., Jan. 10, 2012', $result);
  }

  /**
   * Tests format for same day, same meridian, different start/end time.
   */
  public function testSameDaySameMeridianDifferentTimes() {
    $options = ['display_time' => TRUE, 'time_before_date' => TRUE];

    // Set up a date range on the same day with the same meridian.
    // 10:00 AM.
    $start = new DrupalDateTime('2023-12-15 10:00:00');
    // 11:00 AM
    $end = new DrupalDateTime('2023-12-15 11:00:00');
    ...
    // Assert that the formatted result omits the meridian from the start time.
    $this->assertEquals('10 to 11 a.m., Dec. 15, 2023', $result);
  }

  /**
   * Tests hyphen separator for compact academic/fiscal year ranges.
   */
  public function testHyphenSeparatorAcademicYearRanges() {
    $start = new DrupalDateTime('2023-09-15 00:00:00');
    $end = new DrupalDateTime('2024-06-15 00:00:00');
    $timestamps = [
      'start' => $start->getTimestamp(),
      'end' => $end->getTimestamp(),
    ];
    ...
    $options = ['separator' => 'hyphen'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Sept. 15, 2023-June 15, 2024', $result);

    // Compare with other separators.
    $options = ['separator' => 'to'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Sept. 15, 2023 to June 15, 2024', $result);

    $options = ['separator' => 'endash'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Sept. 15, 2023 &ndash; June 15, 2024', $result);
  }

  /**
   * Tests hyphen separator for short date ranges.
   */
  public function testHyphenSeparatorHeadlineFormat() {
    $start = new DrupalDateTime('2023-09-10 00:00:00');
    $end = new DrupalDateTime('2023-09-15 00:00:00');
    $timestamps = [
      'start' => $start->getTimestamp(),
      'end' => $end->getTimestamp(),
    ];
    ...
    $options = ['separator' => 'hyphen'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Sept. 10-15, 2023', $result);

    // Compare with other separators - hyphen is most space-efficient.
    $options = ['separator' => 'to'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Sept. 10 to 15, 2023', $result);
  }

  /**
   * Tests hyphen separator for time ranges in schedules/timetables.
   */
  public function testHyphenSeparatorScheduleTimeRanges() {
    $start = new DrupalDateTime('2023-12-15 09:00:00');
    $end = new DrupalDateTime('2023-12-15 17:30:00');
    $timestamps = [
      'start' => $start->getTimestamp(),
      'end' => $end->getTimestamp(),
    ];
    ...
    $options = ['display_time' => TRUE, 'separator' => 'hyphen'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Dec. 15, 2023, 9 a.m.-5:30 p.m.', $result);

    $options = ['display_time' => TRUE, 'separator' => 'to'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Dec. 15, 2023, 9 a.m. to 5:30 p.m.', $result);
  }

  /**
   * Tests hyphen separator for month-only ranges in compact formats.
   */
  public function testHyphenSeparatorMonthOnlyRanges() {
    // Month-only ranges.
    $start = new DrupalDateTime('2023-11-01 00:00:00');
    $end = new DrupalDateTime('2024-03-01 00:00:00');
    $timestamps = [
      'start' => $start->getTimestamp(),
      'end' => $end->getTimestamp(),
    ];

    $options = ['month_only' => TRUE, 'separator' => 'hyphen'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Nov. 2023-March 2024', $result);

    $options = ['month_only' => TRUE, 'separator' => 'to'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Nov. 2023 to March 2024', $result);
  }

  /**
   * Tests hyphen separator for same meridian time ranges.
   */
  public function testHyphenSeparatorSameMeridianTimeRanges() {
    $start = new DrupalDateTime('2023-12-15 21:00:00');
    $end = new DrupalDateTime('2023-12-15 22:00:00');
    $timestamps = [
      'start' => $start->getTimestamp(),
      'end' => $end->getTimestamp(),
    ];
    ...
    $options = ['display_time' => TRUE, 'separator' => 'hyphen'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Dec. 15, 2023, 9-10 p.m.', $result);

    $options = ['display_time' => TRUE, 'separator' => 'to'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Dec. 15, 2023, 9 to 10 p.m.', $result);

    $options = ['display_time' => TRUE, 'separator' => 'endash'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Dec. 15, 2023, 9 &ndash; 10 p.m.', $result);

    // Test with morning hours too.
    $start = new DrupalDateTime('2023-12-15 10:00:00');
    $end = new DrupalDateTime('2023-12-15 11:00:00');
    $timestamps = [
      'start' => $start->getTimestamp(),
    ...
    $options = ['display_time' => TRUE, 'separator' => 'hyphen'];
    $result = $this->dateFormatter->formatRange($timestamps, $options);
    $this->assertEquals('Dec. 15, 2023, 10-11 a.m.', $result);
  }
