...
class TrashViewBuilderTest extends TrashKernelTestBase {
  ...
  /**
   * Tests that the exposed bundle filter excludes bundles disabled for trash.
   *
   * @see https://www.drupal.org/project/trash/issues/3612536
   */
  public function testBundleFilterExcludesDisabledBundles(): void {
    // The base setUp() only enables trash for the 'article' bundle, leaving
    // 'page' disabled.
    $executable = $this->container->get(TrashViewBuilder::class)
      ->buildView($this->entityTypeManager->getDefinition('node'));
    $executable->execute('default');
    ...
    assert($exposed_form instanceof ExposedFormPluginInterface);
    $output = $exposed_form->renderExposedForm();
    $html = (string) $this->container->get('renderer')->renderRoot($output);
