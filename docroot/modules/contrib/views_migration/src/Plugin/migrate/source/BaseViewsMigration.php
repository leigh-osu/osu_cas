...
abstract class BaseViewsMigration extends SqlBase implements ViewsMigrationInterface {
   ...
   */
  protected function getSourceHandlerInfoProvider(array $source_data, array $display_options, $base_entity_type) {
   ...
   *
   * @return array
   *   Return Views display options.
   */
  protected function convertDisplayPlugins(array $display_options, $entity_type) {
    $plugin_types = [
    ...
    foreach ($plugin_types as $plugin_type => $source_plugin_id) {
      $instance = $this->getViewsPluginMigratePlugin($plugin_type, $source_plugin_id);
      $instance->entityType = $entity_type;
      $instance->prepareDisplayOptions($display_options);
    }
    if (isset($display_options['menu'])) {
      ...
      if (isset($display_options['menu']['name']) && isset($menu_name_map[$display_options['menu']['name']])) {
   ...
   * @param string $entity_type
   *   Views base entity type.
   * @param array $masterDisplay
   *   Views master display options.
