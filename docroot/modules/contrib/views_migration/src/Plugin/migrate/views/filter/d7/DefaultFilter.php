...
class DefaultFilter extends MigrateViewsHandlerPluginBase {
  ...
  protected function alterOperatorSettings(array &$handler_config) {
    if (isset($handler_config['plugin_id'])) {
      switch ($handler_config['plugin_id']) {
        case 'boolean':
          ...
          elseif (isset($handler_config['value']) && is_array($handler_config['value']) && count($handler_config['value'])) {
            $handler_config['value'] = reset($handler_config['value']);
            if ($handler_config['value'] == 'all') {
              $handler_config['value'] = 1;
            }
        ...
        case 'user_name':
          ...
          if (isset($handler_config['exposed']) && (!isset($handler_config['expose']['operator']) || $handler_config['expose']['operator'] == $handler_config['expose']['operator_id'])) {
    ...
    if (empty($handler_config['operator']) || !isset($handler_config['operator'])) {
