...
class DomainConfigUtilities {
  ...
  /**
   * Returns the domain entity storage.
   *
   * Resolved at the call site rather than held as a property, so a stale
   * storage handler cannot survive a container rebuild.
   *
   * @return \Drupal\domain\DomainStorageInterface
   *   The domain storage handler.
   */
  protected function domainStorage(): DomainStorageInterface {
    return $this->entityTypeManager->getStorage('domain');
  }
