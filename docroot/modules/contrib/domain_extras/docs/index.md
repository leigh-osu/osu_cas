...
### [Domain Render Context](domain_render_context/index.md) (domain_render_context)

Runs a callback as if the request had come from another domain, so output
built outside the request it belongs to (an email, a PDF, a queue item)
carries that domain's links, configuration and path prefix.
