...
class SourcePluginManager extends DefaultPluginManager implements ContextAwarePluginManagerInterface, ContextMatcherPluginManagerInterface {
  ...
  public function getDefinitionsForPropType(string $prop_type_id, ?array $contexts = [], array $tag_filter = []): array {
    ...
    foreach ($definitions as &$definition) {
      $definition['tags'][] = SourceTags::PropTypeNative->value;
    }
    ...
    foreach ($convertibleDefinitions as &$definition) {
      $definition['tags'][] = SourceTags::PropTypeConverted->value;
    }
   ...
   *   Source definitions, keyed by source id.
   */
  private function getNativeDefinitionsForPropType(string $prop_type_id, ?array $contexts = [], array $tag_filter = []): array {
   ...
   */
  private function filterDefinitionsByPropType(array $definitions, string $prop_type_id): array {
    return array_filter($definitions, static function ($definition) use ($prop_type_id) {
      ...
      return !(is_array($supported_prop_types) && (count($supported_prop_types) > 0) && !in_array($prop_type_id, $supported_prop_types, TRUE));
    });
  }
   ...
   *   Source definitions, keyed by source id.
   */
  private function getConvertibleDefinitionsForPropType(string $prop_type_id, ?array $contexts = [], array $tag_filter = []): array {
    $definitions = [];
   ...
   */
  private function getConvertibleDefinitionsPerPropertyId(string $prop_type_id, ?array $contexts = [], ?array $tag_filter = []): array {
    $definitions = [];
    if (!is_array($tag_filter)) {
      $tag_filter = [];
    }
    $tag_filter = array_merge($tag_filter, [SourceTags::WidgetDismissible->value => FALSE]);
   ...
   */
  private function isApplicable(string $source_id, array $contexts): bool {
