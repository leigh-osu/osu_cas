...
class CompatibilityChecker {
   ...
   *
   * @param \Drupal\ui_patterns\SchemaManager\Canonicalizer $canonicalizer
   *   Canonicalizer.
   * @param \Drupal\ui_patterns\SchemaManager\ReferencesResolver $resolver
   *   JSON schema references recursive resolver.
   */
  public function __construct(
    protected Canonicalizer $canonicalizer,
    protected ReferencesResolver $resolver,
  ) {}
  ...
  public function isCompatible(array $checked_schema, array $reference_schema): bool {
    ...
    // The schema to check may be already resolved by ComponentPluginManager,
    // but we are not sure, so let's run the resolver again.
    $checked_schema = $this->resolver->resolve($checked_schema);
    // The reference schema has not been resolved by PropTypePluginManager or
    // PropTypeAdapterPluginManager, so let's do it.
    $reference_schema = $this->resolver->resolve($reference_schema);
    ...
    if (isset($checked_schema['type'], $reference_schema['type'])) {
  ...
  protected function isObjectCompatible(array $checked_schema, array $reference_schema): bool {
    ...
    if (isset($checked_schema['properties']) && !$this->areObjectPropertiesCompatible($checked_schema, $reference_schema)) {
      return FALSE;
    }
  ...
  /**
   * Is the schema rejecting additional properties?
   */
  protected function isRejectingAdditionalProperties(array $schema): bool {
    return isset($schema['additionalProperties']) && $schema['additionalProperties'] === FALSE;
  }

  /**
   * Are the object properties compatible?
   */
  protected function areObjectPropertiesCompatible(array $checked_schema, array $reference_schema): bool {
    $no_additional = $this->isRejectingAdditionalProperties($reference_schema);
    foreach ($checked_schema['properties'] ?? [] as $property => $checked_value) {
      $reference_value = $reference_schema['properties'][$property] ?? NULL;
      ...
      if (!$this->isCompatible($checked_value, $reference_value)) {
        return FALSE;
      }
    }

    foreach ($checked_schema['patternProperties'] ?? [] as $property => $checked_value) {
      $reference_value = $reference_schema['patternProperties'][$property] ?? NULL;
      if ($reference_value === NULL) {
        if ($no_additional) {
          return FALSE;
        }
  ...
  protected function isArrayCompatible(array $checked_schema, array $reference_schema): bool {
    ...
    if (isset($checked_schema['items'], $reference_schema['items'])) {
