 ...
 */
#[ConfigAction(
  id: 'entityViewModeRestrictionByRegionAppendAllowedBlocks',
        ...
        foreach ($regions as $region) {
          foreach ($value["blocks"] as $block_category => $block_ids) {
            foreach ($block_ids as $block_id) {
              if (!isset($third_party_settings["layout_builder_restrictions"]["entity_view_mode_restriction_by_region"]["allowlisted_blocks"][$layout][$region][$block_category])) {
                $third_party_settings["layout_builder_restrictions"]["entity_view_mode_restriction_by_region"]["allowlisted_blocks"][$layout][$region][$block_category] = [];
              }
              if (!in_array($block_id, $third_party_settings["layout_builder_restrictions"]["entity_view_mode_restriction_by_region"]["allowlisted_blocks"][$layout][$region][$block_category])) {
                $third_party_settings["layout_builder_restrictions"]["entity_view_mode_restriction_by_region"]["allowlisted_blocks"][$layout][$region][$block_category][] = $block_id;
              }
            }
          }
        }
