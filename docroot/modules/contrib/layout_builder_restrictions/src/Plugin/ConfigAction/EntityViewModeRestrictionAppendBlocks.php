 ...
 */
#[ConfigAction(
  id: 'entityViewModeRestrictionAppendAllowedBlocks',
  ...
  public function apply(string $configName, mixed $values): void {
    ...
    foreach ($values["blocks"] as $block_category => $block_ids) {
      foreach ($block_ids as $block_id) {
        if (!isset($third_party_settings["layout_builder_restrictions"]['entity_view_mode_restriction']["allowlisted_blocks"][$block_category])) {
          $third_party_settings["layout_builder_restrictions"]['entity_view_mode_restriction']["allowlisted_blocks"][$block_category] = [];
        }
        if (!in_array($block_id, $third_party_settings["layout_builder_restrictions"]['entity_view_mode_restriction']["allowlisted_blocks"][$block_category])) {
          $third_party_settings["layout_builder_restrictions"]['entity_view_mode_restriction']["allowlisted_blocks"][$block_category][] = $block_id;
        }
      }
    }
