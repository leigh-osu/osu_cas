...
$settings['config_sync_directory'] = '../config/endophyte.emt.oregonstate.edu';
...
if (file_exists('/var/www/site-php')) {
  ...
  // The Acquia database is named "endophytelab", not the long form
  ...
  require "/var/www/site-php/{$_ENV['AH_SITE_GROUP']}/endophytelab-settings.inc";
 ...
 *
 * The lab mails clients on workflow transitions -- a sample arriving, a test
 * being finalised, a certificate becoming available. Any environment holding a
 * copy of this database will therefore mail real growers and seed companies
 * the moment someone advances a sample. reroute_email catches every outbound
 * message and sends it to the single address in reroute_email.settings.
 *
 * Expressed as "reroute unless prod" rather than "reroute on dev and stage" so
 * both failure modes land on the safe side: a new environment nobody thought
 * about reroutes by default, and a database copied from production to stage
 * starts rerouting again the moment it is served from a non-production
 * environment, without anyone remembering to change a setting.
 *
 ...
 */
$osu_esl_ah_env = $_ENV['AH_SITE_ENVIRONMENT'] ?? '';
$config['reroute_email.settings']['enable'] = ($osu_esl_ah_env !== 'prod');
 ...
 *
 * discarding the value set earlier in this file. On agsci that silently aimed
 * `drush cex` and `drush cim` at ../config/default on every hosted
 * environment, and made cron log "Security warning: Couldn't write .htaccess
 * file" every fifteen minutes.
 *
